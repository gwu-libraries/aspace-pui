...
if [ "$ASPACE_JAVA_XMS" = "" ]; then
    ASPACE_JAVA_XMS="-Xms1024m"
fi

if [ "$ASPACE_JAVA_NEWSIZE" = "" ]; then
    ASPACE_JAVA_NEWSIZE="-XX:NewSize=512m"
fi

if [ "$ASPACE_JAVA_MAXNEWSIZE" = "" ]; then
    ASPACE_JAVA_MAXNEWSIZE="-XX:MaxNewSize=512m"
fi

#if [ "$ASPACE_JAVA_METASPACESIZE" = "" ]; then
#    ASPACE_JAVA_METASPACESIZE="-XX:MetaspaceSize=512m"
#fi

#if [ "$ASPACE_JAVA_MAXMETASPACESIZE" = "" ]; then
#    ASPACE_JAVA_MAXMETASPACESIZE="-XX:MaxMetaspaceSize=1024m"
#fi

if [ "$ASPACE_JAVA_ASPACE_GC_OPTS" = "" ]; then
    ASPACE_GC_OPTS="-XX:+CMSClassUnloadingEnabled -XX:+UseG1GC -XX:NewRatio=1 -XX:ConcGCThreads=5 -XX:MaxGCPauseMillis=200 -XX:+DisableExplicitGC -XX:ParallelGCThreads=20"
fi
...
startup_cmd="java "$JAVA_OPTS"  \
        $ASPACE_GC_OPTS $ASPACE_JAVA_XMS $ASPACE_JAVA_NEWSIZE $ASPACE_JAVA_MAXNEWSIZE $ASPACE_JAVA_METASPACESIZE $ASPACE_JAVA_MAXMETASPACESIZE $ASPACE_JAVA_XMX $ASPACE_JAVA_XSS $ASPACE_JAVA_MAXPERMSIZE -Dfile.encoding=UTF-8 \
